...
function StepCrumbs({ step = 1 }: { step?: 1 | 2 | 3 }) {
  ...
  return (
    <nav className="mb-5 flex items-center gap-2 text-xs text-neutral-600" aria-label="Onboarding steps">
      {labels.map((label, i) => (
        <span key={label} className="flex items-center gap-2">
          <span
            className={`inline-flex h-6 items-center rounded-md px-2 ${i + 1 <= step ? "bg-neutral-100 font-medium" : "bg-neutral-50"}`}
          >
            {label}
          </span>
              ...
              return (
                <label key={r.id} className={`flex items-start gap-2 rounded-lg border p-2 text-sm ${disabled ? "opacity-50" : ""}`}>
                  ...
                  <div>
                    ...
                    <div className="text-neutral-600">{r.desc}{disabled ? " (Industry only)" : ""}</div>
                  </div>
                </label>
              );
            })}
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="first" className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.first} onChange={(e) => update("first", e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium" htmlFor="last">Last Name *</label>
              <input id="last" className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.last} onChange={(e) => update("last", e.target.value)} />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium" htmlFor="email">Email Address *</label>
            <input id="email" type="email" className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.email} onChange={(e) => update("email", e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium" htmlFor="org">Organization</label>
            <input id="org" className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.org} onChange={(e) => update("org", e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium" htmlFor="inq">Inquiry Type *</label>
            <select id="inq" className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.inquiry} onChange={(e) => update("inquiry", e.target.value)}>
          ...
          <div>
            ...
            <textarea id="msg" rows={5} className="mt-1 w-full rounded-md border border-neutral-300 p-2 text-sm" value={form.message} onChange={(e) => update("message", e.target.value)} />
          </div>
...
export default function JoinWireframe() {
    ...
    <div className="min-h-screen bg-white text-neutral-900">
      <div className="mx-auto max-w-6xl px-4 py-10">
        <header className="mb-8 text-center">
          <p className="text-xs font-semibold uppercase tracking-wide text-neutral-500">Contact / Waitlist</p>
          <h1 className="mt-1 text-3xl font-bold">Join Industry Commons for Defense</h1>
          <p className="mt-3 mx-auto max-w-2xl text-sm text-neutral-600">
            ...
            {categories.map((c) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-neutral-100 text-lg">{c.icon}</div>
                ...
                <p className="mt-3 text-xs text-neutral-500">{c.subtext}</p>
        ...
        <section id="faq" className="mt-16">
          ...
          <div className="mx-auto mt-6 grid max-w-4xl gap-4">
            <details className="rounded-xl border border-neutral-200 bg-white p-4">
              ...
              <p className="mt-2 text-sm text-neutral-700">A neutral, nonprofit-led framework that enables government and industry to build software together under a shared charter—protecting IP, speeding delivery, and reducing duplication.</p>
            </details>
            <details className="rounded-xl border border-neutral-200 bg-white p-4">
              <summary className="cursor-pointer select-none text-sm font-semibold">How does the waitlist work for Industry?</summary>
              <p className="mt-2 text-sm text-neutral-700">Industry applications are accepted now and placed on a waitlist until initial Steering Board members are selected. You’ll receive updates and opportunities to contribute components and pilots in the meantime.</p>
            </details>
            <details className="rounded-xl border border-neutral-200 bg-white p-4">
              <summary className="cursor-pointer select-none text-sm font-semibold">Can government partners join today?</summary>
              <p className="mt-2 text-sm text-neutral-700">Yes. Government partners can join and participate in pilots, provide governance input, and adopt the Charter to reduce duplication.</p>
            </details>
            <details className="rounded-xl border border-neutral-200 bg-white p-4">
              ...
              <p className="mt-2 text-sm text-neutral-700">End Users deploy and benefit from components; Contributors share code/data modules; Steering Board members (limited Industry seats) provide oversight and investment.</p>
            </details>
            <details className="rounded-xl border border-neutral-200 bg-white p-4">
              <summary className="cursor-pointer select-none text-sm font-semibold">Where can I read the Charter?</summary>
              <p className="mt-2 text-sm text-neutral-700">We’ll publish the Defense Commons Charter on this site. In the interim, request it via the contact form and we’ll send the latest draft.</p>
            </details>
          </div>
        </section>

        <footer className="mt-16 text-center text-sm text-neutral-500">
          <p>FAQ and more links live above. This page is a middle ground between our legacy contact form and the new role-based waitlist.</p>
        </footer>
      </div>
    </div>
  );
